...
class FormatStep extends React.Component {
          ...
          checkboxes.map(
            x => (
              <div className="col-md-6 col-sm-12" key={x.name}>
                <Checkbox label={x.name} checked={x.isChecked}
  ...
  getCheckedOptionsDataSource = () => {
    return this.state.allOptions.filter(x => x.isChecked)
      .map(x => {
        return {
          'data-point': x.name,
          'sort': 'Not Sorted',
        ...
        <div className="row">
          <div className="col-xl-6 col-lg-12">
            <Collapse accordion>
              ...
              {this.renderOptionsPanel("All Custom Fields", "all-custom-fields",
                [
                  {options: this.getCustomFieldsOptions()}
                ])}
              {this.renderOptionsPanel("Segments", "segments",
                [
                  {options: this.getTraining1Options(), label: "Training 1", key: "training-1"},
                  {options: this.getTraining2Options(), label: "Training 2", key: "training-2"}
                ])}
            </Collapse>
          </div>
          <div className="col-xl-6 col-lg-12">
