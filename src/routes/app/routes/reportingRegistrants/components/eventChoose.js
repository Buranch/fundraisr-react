...
class ChooseEvents extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <h5>Choose Events</h5>
        <small>You may retrieve data in your report for one or more events. Select the Events you wish to report on,
          then Add the selections to the Selected Table by clicking the "Add" Button
        </small>
        <div className="text-center" style={controlPanelStyle}>
          <FlatButton onClick={this.toggleIndividual} style={mWidthStyle} label="Choose Individual Initiatives"
                      secondary={this.state.individual}/>
          <FlatButton onClick={this.toggleSpecifyType} style={mWidthStyle} label="Specify the type of Initiative"
                      secondary={!this.state.individual}/>
        </div>
        {
        ...
        />
        <FlatButton icon={<AddIcon/>} secondary onClick={this.start} disabled={!hasSelected} style={mWidthStyle}
                    label="Add All"/>
